...
select isim,urun_adi ,sum (urun_miktari)as topla_urun from manav
...
select cinsiyet, avg (maas)  from personel
group by cinsiyet
...
select ulke,cinsiyet,count(*)as calisan_sayisi, avg(yas)
...
select ulke,cinsiyet,avg(yas),as ortalama_yas, count(*) from personel
where maas>30000 group by ulke,cinsiyet
